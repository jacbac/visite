        ...
        this.$get = ['$animation', '$window', '$sniffer', function($animation, $window, $sniffer) {
            return function(scope, attrs) {
                ...
                function animateActionFactory(type, beforeFn, afterFn) {
                    ...
                    var className = ngAnimateAttr ? isObject(ngAnimateValue) ? ngAnimateValue[type] : ngAnimateValue + '-' + type : '';
                            ...
                            function beginAnimation() {
                                ...
                                if (polyfillStart) {
                                    ...
                                    forEach(element, function(element) {
                                        ...
                                        duration = Math.max(
                                        parseFloat(globalStyles[w3cTransitionProp + durationKey]) || parseFloat(globalStyles[vendorTransitionProp + durationKey]) || 0,
                                        duration);
                                    });
        ...
        if (csp) {
            fn = (pathKeysLength < 6) ? cspSafeGetterFn(pathKeys[0], pathKeys[1], pathKeys[2], pathKeys[3], pathKeys[4]) : function(scope, locals) {
                var i = 0,
                    ...
                    do {
                        val = cspSafeGetterFn(
                        pathKeys[i++], pathKeys[i++], pathKeys[i++], pathKeys[i++], pathKeys[i++])(scope, locals);
 ...
 */
    ...
    forEach('click dblclick mousedown mouseup mouseover mouseout mousemove mouseenter mouseleave keydown keyup keypress'.split(' '),
 ...
 */
     ...
     */
    var ngViewDirective = ['$http', '$templateCache', '$route', '$anchorScroll', '$compile', '$controller', '$animator',
